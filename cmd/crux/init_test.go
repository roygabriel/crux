...
// writeTestConfigWithAgent writes a minimal config.yaml with a claude agent
// to the given path. The project root is set to the directory of cfgPath's parent.
func writeTestConfigWithAgent(t *testing.T, cfgPath string) {
	t.Helper()
	dir := filepath.Dir(filepath.Dir(cfgPath)) // .crux -> project root
	if dir == "." {
		var err error
		dir, err = os.Getwd()
		if err != nil {
  ...
  name: test-project
  root: "` + dir + `"
  state_dir: "` + filepath.Dir(cfgPath) + `"
agents:
  dev:
    plugin: claude
    role: engineer
    permission: standard
memory:
  sqlite_path: "` + filepath.Join(filepath.Dir(cfgPath), "memory.db") + `"
  vector_dir: "` + filepath.Join(filepath.Dir(cfgPath), "vectors") + `"
phases:
  spec_dir: docs/phases
security:
  audit_log: "` + filepath.Join(filepath.Dir(cfgPath), "audit.log") + `"
  max_cmds_per_min: 60
...
func TestRunDefaultInit_GeneratesInstructionFiles(t *testing.T) {
	dir := t.TempDir()
	origDir, err := os.Getwd()
	if err != nil {
		t.Fatal(err)
	...
	cruxDir := filepath.Join(dir, ".crux")
	for _, d := range []string{cruxDir, "docs/phases", "work-notes", "templates"} {
	...
	writeTestConfigWithAgent(t, filepath.Join(dir, ".crux", "config.yaml"))

	if err := runDefaultInit(cfgPath); err != nil {
		t.Fatalf("runDefaultInit: %v", err)
	}

	// CLAUDE.md should exist in project root (generated by claude adapter).
...
func TestRunDefaultInit_SkipsInstructWhenFlagSet(t *testing.T) {
	dir := t.TempDir()
	origDir, err := os.Getwd()
	if err != nil {
		t.Fatal(err)
	...
	// CLAUDE.md should NOT exist since we skipped instruct.
	claudeMD := filepath.Join(dir, "CLAUDE.md")
	if _, err := os.Stat(claudeMD); err == nil {
		t.Error("CLAUDE.md should not exist when --skip-instruct is set")
	}
}

func TestRunDefaultInit_InitializesMemoryBank(t *testing.T) {
	dir := t.TempDir()
	origDir, err := os.Getwd()
	if err != nil {
		t.Fatal(err)
	...
	skipPrefs = true
	skipPlan = true
	skipInstruct = true // Skip instruct to focus on memory bank.
	...
	// projectBrief.md should exist in memory bank directory.
	briefPath := filepath.Join(cruxDir, "memory-bank", "projectBrief.md")
	if _, err := os.Stat(briefPath); err != nil {
		t.Errorf("memory bank projectBrief.md not created: %v", err)
	}
}
