...
func TestPluginName(t *testing.T) {
	...
	if got := newPlugin().Name(); got != "codex" {
...
func TestDetectRateLimit(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:        "quota-exceeded",
			paneContent: "Quota exceeded, please try again later\n",
			wantLimited: true,
			wantMin:     60 * time.Second,
		},
		{
...
func TestParseOutput(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			paneContent:  "Created `internal/foo/bar.go`\nApplied edit to `internal/foo/baz.go`\n>\n",
			wantFiles:    []string{"internal/foo/bar.go", "internal/foo/baz.go"},
			wantComplete: true,
		},
		{
			name:         "wrote-pattern",
			paneContent:  "Wrote `main.go`\nWrote to `lib/util.go`\n>\n",
			wantFiles:    []string{"main.go", "lib/util.go"},
			wantComplete: true,
		},
		{
			name:         "with-errors",
