...
func TestPluginName(t *testing.T) {
	...
	if got := newPlugin().Name(); got != "gemini" {
...
func TestDetectError(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			wantMsg:     "access denied to resource",
			wantErr:     true,
		},
		{
			name:        "api-error",
...
func TestDetectRateLimit(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			paneContent: "Resource exhausted, try again later\n",
			wantLimited: true,
			wantMin:     60 * time.Second,
		},
		{
			name:        "quota-exceeded",
			paneContent: "Quota exceeded for project\n",
			wantLimited: true,
			wantMin:     60 * time.Second,
		},
		{
